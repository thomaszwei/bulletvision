...
const SETTING_METAS: SettingMeta[] = [
  // Detection
  { key: "confidence_threshold", label: "Confidence Threshold", description: "Minimum confidence to report a detection (0–1)", type: "number", min: 0.1, max: 0.99, step: 0.01, group: "Detection" },
  { key: "min_area", label: "Min Contour Area (px²)", description: "Smallest contour area counted as a hole", type: "number", min: 10, max: 5000, step: 10, group: "Detection" },
  { key: "max_area", label: "Max Contour Area (px²)", description: "Largest contour area counted as a hole", type: "number", min: 100, max: 50000, step: 100, group: "Detection" },
  { key: "detection_fps", label: "Detection FPS", description: "Frames per second to analyse (1–10)", type: "number", min: 1, max: 10, step: 1, group: "Detection" },
...
export default function Settings() {
  ...
  // Merge remote into local on first load
  useEffect(() => {
    if (!isLoading) {
      setLocal(remote);
      setDirty(false);
    ...
    <div className="space-y-6 animate-fade-in max-w-2xl">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 text-sm">Detection, camera and scoring parameters</p>
        </div>
        {dirty && (
          <div className="flex gap-2">
            <button className="btn-ghost" onClick={handleReset} disabled={saveMutation.isPending}>
      ...
      {saveMutation.isError && (
        <p className="text-accent text-sm">Failed to save settings. Please try again.</p>
      )}
    </div>
  );
}
